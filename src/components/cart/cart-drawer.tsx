  ...
  return (
    ...
    >
      ...
      <aside
        ...
        className={`absolute right-0 top-0 flex h-full w-full max-w-md flex-col bg-white shadow-xl transition-transform duration-300 dark:bg-zinc-950 ${
          isOpen ? "translate-x-0" : "translate-x-full"
        }`}
      >
        <div className="flex items-center justify-between border-b border-black/10 px-5 py-4 dark:border-white/10">
          <h2 className="text-base font-semibold">
            Your cart {itemCount > 0 && <span className="text-zinc-400">({itemCount})</span>}
          </h2>
          <button
            type="button"
            onClick={closeCart}
        ...
        {error && (
          <p className="mx-5 mt-3 rounded-md bg-red-50 px-3 py-2 text-sm text-red-700 dark:bg-red-950 dark:text-red-300">
            {error}
          </p>
        )}
                    ...
                    <div className="mt-2 flex items-center justify-between">
                      <div className="flex items-center rounded-md border border-black/10 dark:border-white/15">
                        ...
                        <button
                          ...
                          onClick={() => setItemQuantity(item.productId, item.quantity + 1)}
                          className="px-2 py-1 text-sm hover:bg-black/5 disabled:opacity-40 dark:hover:bg-white/10"
                        >
                          +
                        </button>
                      </div>
